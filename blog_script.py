...
for filename in files:
    ...
    data_m.at[year, '_vc_ratio_lump'] = 0
    data_m.at[year, '_sum'] = 0

    data_f.at[year, '_length_lump'] = 0
    data_f.at[year, '_vc_ratio_lump'] = 0
    data_f.at[year, '_sum'] = 0

    print(f'examining names in {year}...')
    f = open(os.path.join(datapath, filename), 'r')
    for line in f:
        name_data_t = line.strip().lower().split(',')
        name, number = (name_data_t[0], int(name_data_t[2]))

        vowel_num = len([c for c in name if c in vowels])
        vc_ratio = vowel_num / len(name)

        if name_data_t[1] == 'M':
            data_m.at[year, name] = number
            data_m.at[year, '_length_lump'] += len(name) * number
            data_m.at[year, '_vc_ratio_lump'] += vc_ratio * number
            data_m.at[year, '_sum'] += number
        else:
            data_f.at[year, name] = number
            data_f.at[year, '_length_lump'] += len(name) * number
            data_f.at[year, '_vc_ratio_lump'] += vc_ratio * number
            data_f.at[year, '_sum'] += number
    f.close()

data_m['_avg_length'] = data_m['_length_lump'] / data_m['_sum']
data_m['_avg_vc_ratio'] = data_m['_vc_ratio_lump'] / data_m['_sum']
data_m = data_m.sort_index(axis=1)

data_f['_avg_length'] = data_f['_length_lump'] / data_f['_sum']
data_f['_avg_vc_ratio'] = data_f['_vc_ratio_lump'] / data_f['_sum']
data_f = data_f.sort_index(axis=1)

plt.figure()
ax1 = plt.axes()
ax1.plot(year_range, data_m['_avg_length'], label='M')
ax1.plot(year_range, data_f['_avg_length'], label='F')
ax1.legend(loc='upper left')
ax1.set_xlabel('Year')
ax1.set_ylabel('Average first name length (letters)')
plt.show()

plt.figure()
ax2 = plt.axes()
ax2.plot(year_range, data_m['_avg_vc_ratio'], label='M')
ax2.plot(year_range, data_f['_avg_vc_ratio'], label='F')
ax2.legend(loc='upper left')
ax2.set_xlabel('Year')
ax2.set_ylabel('Average first name vowel/consonant ratio')
plt.show()
